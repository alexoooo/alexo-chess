...
//    private final static int reportPeriod = 1_000;
    ...
    public static class Builder
    {
        ...
        public Builder(MetaModel model) {
            this.model = model;
        }
        ...
        public MetaPlayer build()
        {
            return new MetaPlayer(
                    model,
                    threads,
                    exploration,
                    explorationLog,
    ...
    private final MetaModelPool pool;
    ...
    private final ConcurrentHashMap<Long, MetaEstimate> nnCache = new ConcurrentHashMap<>();
    ...
    private final CopyOnWriteArrayList<MetaContext> contexts;
    private ExecutorService executorService;

    private long previousPositionHash = -1;
    private MetaNode previousRoot;

    private final long[] history;
    private int historyIndex;

    private State prevState = null;
    private MetaNode prevPlay  = null;
            ...
            boolean useIo)
    {
        ...
        pool = new MetaModelPool(
                threads, model,
                MetaNode.guessRange, MetaNode.minimumGuess);
    ...
    public int move(
        ...
        MetaNode root = getOrCreateRoot(position, positionHash);
        ...
        if (prevState != null && prevPlay != null) {
            ...
            if (move != -1) {
                ...
                if (index != -1) {
                    MetaNode child = prevPlay.childOrNull(index);
                    if (child != null) {
        ...
        if (threads == 1) {
            MetaContext context = contexts.get(0);
            do {
                thinkingEpisode(
            ...
            for (int thread = 0; thread < threads; thread++) {
                ...
                MetaContext context = contexts.get(thread);
                MetaNode currentRoot = root;
...
//                    predictionUncertainty,
    ...
    private MetaNode getOrCreateRoot(State state, long positionHash) {
...
//            display("Retrieved root" + previousRoot);
        ...
        MetaEstimate estimate = contexts.get(0).pool.estimateRoot(state, legalMoves);
...
//        display("Created root: " + root);
    ...
    private void reportProgress(
        ...
        String generalPrefix = String.format(
                "%s - %s | %d / %.2f / %b / %b | u%d / c%d / x%d / t%d / e%d | %s",
                id,
                model,
                threads,
                exploration,
