...
import ao.chess.v2.engine.neuro.rollout.store.*;
...
public class CompactTieredRolloutStore implements RolloutStore {
    //-----------------------------------------------------------------------------------------------------------------
    private final CompactFileRolloutWriteStore writer;
    private final CompactFileRolloutReadStore reader;
    ...
    //-----------------------------------------------------------------------------------------------------------------
    public CompactTieredRolloutStore(
            Path file,
            Path transpositionFile,
            int readHandleCount,
            int writeHandleCount
    ) {
        writer = new CompactFileRolloutWriteStore(file, writeHandleCount);
        reader = new CompactFileRolloutReadStore(file, readHandleCount);
    ...
    public synchronized long expandChildIfMissing(long nodeIndex, int moveIndex, int childMoveCount) {
        ...
        int childSize = CompactFileRolloutStore.sizeOf(childMoveCount);
