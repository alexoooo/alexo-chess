...
//    private static final int miniBatchSize = 1024;
    ...
    private static final int testBatchSize = 32;
//    private static final int testBatchSize = 512;
//    private static final boolean testBatching = false;
//    private static final boolean testBatching = true;
...
//            Paths.get("lookup/nn/res_20.zip");
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Training " + saveFile + " - " + saved + " - " + miniBatchSize);
    ...
    private static List<MoveAndOutcomeProbability> testExampleAll(ComputationGraph nn, List<MoveHistory> examples) {
        ...
        for (var batch : Lists.partition(examples, testBatchSize))
        {
            if (batch.size() == testBatchSize) {
                all.addAll(testEncoder.estimateAll(batch, nn));
            }
            else {
                for (var example : batch) {
                    all.add(testEncoder.estimate(example, nn));
                }
            }
        }
        return all;
...
//        builder.addValueHead(body, 64, 384);
    ...
    public static ComputationGraph createResidualNetwork14b() {
...
//    }
    ...
    public static ComputationGraph createResidualNetwork5Meta() {
        NnResBuilder builder = new NnResBuilder(
                256, Activation.RELU, false, true);
